...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR CODE HERE
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, 'policy_net', self.n_layers, self.size)
            return sy_logits_na
        else:
            # YOUR CODE HERE
            sy_mean = build_mlp(sy_ob_no, self.ac_dim, 'policy_net', self.n_layers, self.size)
            sy_logstd = tf.get_variable('sy_logstd', [self.ac_dim], tf.float32)
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR CODE HERE
            sy_sampled_ac = tf.reshape(tf.multinomial(sy_logits_na, 1, output_dtype=tf.int32), [-1])
        else:
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            ...
            q_m = [[np.sum([self.gamma**i * r for i, r in enumerate(re)])] * len(re) for re in re_n]
        q_n = np.concatenate(q_m)
        return q_n
    ...
    def estimate_return(self, ob_no, re_n):
        ...
        if self.normalize_advantages:
            ...
            adv_n = self.norm(adv_n) # YOUR CODE HERE
        return q_n, adv_n
    ...
    def norm(self, value, mean=0., std=1.):
        ...
        normalized_value = normalized_value * std + mean
